...
/** 與伺服器 `MAX_OLLAMA_VISION_IMAGES` 對齊。 */
...
type ChatTurn = {
  role: 'user' | 'assistant'
  content: string
  /** 若為 true，之後送 API 時此則 user 改為空字串（僅該輪曾帶參考圖）。 */
                  ...
                  {t.role === 'user' ? (
                    <>
                      ...
                      {t.userImageDataUrls && t.userImageDataUrls.length > 0 ? (
                        <div className="cta__user-imgs" role="group" aria-label="此則訊息附上的參考圖">
                          {t.userImageDataUrls.map((url, idx) => (
                            <img
                              key={`${String(idx)}-${url.slice(0, 48)}`}
                              className="cta__user-img"
                              src={url}
                              alt=""
                            />
                          ))}
            ...
            <div className="cta__field">
              ...
              >
                {pendingImages.length > 0 ? (
                  <ul className="cta__pending-list" aria-label="待送出參考圖預覽">
                    {pendingImages.map((p) => (
                      <li key={p.id} className="cta__pending-item">
                        <img className="cta__drop-preview" src={p.dataUrl} alt="" />
                        <button
                          type="button"
                          className="cta__pending-remove"
                          onClick={(e) => {
                            e.stopPropagation()
                  ...
                  <span className="cta__drop-placeholder">點此或拖放圖片（可多張，最多 {MAX_PENDING_IMAGES}）</span>
                )}
              </div>
              <input
                ref={fileInputRef}
              ...
              />
              {pendingImages.length > 0 ? (
                <button type="button" className="cta__ghost" onClick={clearPendingImages}>
                  清空待送圖
                </button>
              ) : null}
            </div>
